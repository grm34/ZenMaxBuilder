...
url="https://apis.dashboard.techspecs.io/7s2kibnwqb1dy2yl/en/v3/product/search?query="
[[ ! -d temp ]] && mkdir temp
...
_search_devices() {
  curl -s -L -o temp/query.json \
    --request POST \
    --url "${url}${2// /%20}" \
    --header 'Accept: application/json' \
    --header 'Content-Type: application/json' \
    --header 'x-blobr-key: EAJIGl4C5ZQTkohu8DNlQoXCYCWGNP42' \
    --data '{"category":"all"}'
  if grep -sqm 1 "model" temp/query.json; then
    local device index
    _find_devices "model" "version"
    # shellcheck disable=SC2154
    for device in "${!model[@]}"; do
      _print_devices "$(( device + 1 ))" \
        "${model[device]}" "${version[device]}"
    done
    ...
    if grep -sqm 1 model temp/device.json; then
      _find_device_specs
    else
